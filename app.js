...
/* ===== WS4 Logger – all prospect persistence (Calculator Database) ===== */
const WS4_LOG = "https://script.google.com/macros/s/AKfycbx146CZOaBxUg2fKGZKyGiTGKQbpcM7CgFtCU01boTLgh6JhaksMgZKSYx4oeeujrS7pA/exec";
...
function getAttribution(){
 ...
 try {
  ...
  UTM_KEYS.forEach(k => { const v = params.get(k); if (v) att[k] = v; });
  if (Object.keys(att).length) att.source = "mortgage-calculator";
  const stored = localStorage.getItem("azm_attribution");
  if (stored) try { att = { ...JSON.parse(stored), ...att }; } catch { /* ignore */ }
  if (Object.keys(att).length) localStorage.setItem("azm_attribution", JSON.stringify(att));
 } catch { /* ignore */ }
 return att;
}
/* ---------- Tax/HOI ---------- */
...
function pillCheckbox(id, label, checked, onToggle){
 const wrap=document.createElement("div");
 wrap.className="pill" + (checked ? " pill-selected" : "");
 wrap.setAttribute("role","button");
 wrap.setAttribute("tabindex","0");
 wrap.setAttribute("aria-pressed", checked);
 const i=document.createElement("input");
 i.type="checkbox"; i.id=id; i.checked=!!checked;
 i.className="pill-input-hidden";
 const s=document.createElement("span"); s.textContent=label;
 wrap.appendChild(i); wrap.appendChild(s);
 const toggle=()=>{
  i.checked=!i.checked;
  wrap.classList.toggle("pill-selected",i.checked);
  wrap.setAttribute("aria-pressed",i.checked);
  if (typeof onToggle==="function") onToggle();
  renderLoanLine();
  maybeReprice();
 };
 wrap.addEventListener("click", (e)=>{ e.preventDefault(); toggle(); });
...
function renderProgFactors(){
 ...
 if(kind==="CONV"){
  if (txn==="PURCHASE"){
   ...
   wrap.appendChild(pillCheckbox("firstTimeBuyer","First‑Time Homebuyer",false, firstTimeBuyerOnToggle));
  } else if (txn==="REFI"){
   wrap.appendChild(pillCheckbox("twoPlusBorrowers","2+ Borrowers",false));
  } else if (txn==="CASHOUT"){
   // no factors for CASHOUT
  }
...
/** Only show Re-Calculate message when gate is unlocked; otherwise show Get My Results (same rule as btnGate visibility). */
function showRecalcOrGateStatus(){ showStatus(gated ? "Click Re-Calculate to Update Pricing." : "Pricing ready — click Get My Results to review.", "info"); }
function markCoreDirty(){ coreDirty = true; setRecalcState(); showRecalcOrGateStatus(); }
...
function currentInputs(){
 ...
 const pmiToggle=($("pmiToggle")?.checked) ?? showIncludePmi();
...
/* ---------- Logger (non-blocking) ---------- */
async function logRunToQuotes({ inputs, quoteWith, quotePar }) {
  try {
    if (!WS4_LOG) return; // no-op if not configured
    const leadToken = localStorage.getItem("azm_leadToken") || "";
    const leadMeta = {
    ...
    const payload = {
      ...
      payload: {
        ...
        quote: quoteWith || {},
        parQuote: quotePar || null,
        leadMeta,
        attribution: getAttribution()
      }
    };
    // Write to dedicated Logger dev pane
    captureDev("devLogReq", payload);
    const res = await fetch(WS4_LOG, {
      method: "POST",
    ...
    captureDev("devLogRes", { status: res.status, statusText: res.statusText, body: text }, 2000);
    let data;
    try { data = JSON.parse(text); } catch { data = null; }
    if (!res.ok || !(data && data.ok)) {
      if (DEV_SHOW) toast("Logger: request failed (check dev pane).", "warn", 3000);
    }
  } catch (err) {
    captureDev("devLogRes", String(err));
    if (DEV_SHOW) toast("Logger: network error (check dev pane).", "warn", 3000);
  }
}
...
async function priceNow(){
 ...
 if(quoteWith){
  lastQuote = quoteWith; // keep Email button available when switching points (cached)
 ...
 try{
  if(!quoteWith){
   ...
   if (data?.quotes && data?.context){
    ...
    logRunToQuotes({ inputs, quoteWith, quotePar: q["0"] });
  ...
  if(quotePar){
   ...
   // Log run
   logRunToQuotes({ inputs, quoteWith, quotePar });
  ...
  if(onlyPointsChanged){
   ...
   // Log run
   logRunToQuotes({ inputs, quoteWith, quotePar: lastQuotePar });
...
/* ---------- LIVE refresh helper ---------- */
/** Allow live refresh when gated, regardless of coreDirty. 
 * Callers decide when live updates are appropriate (e.g., points, DSCR, taxes when not core-dirty). */
...
/* ---------- Email Quote ---------- */
function saveQuote(){
 if(!gated||!leadToken||!lastQuote){ showStatus("Nothing to email yet.","warn"); toast("Nothing to email yet","warn"); return; }
 openEmailModal();
}
function openEmailModal(){
 const stored = localStorage.getItem("azm_leadEmail") || "";
 $("emailConfirm").value = stored;
 $("emailConfirmErr").style.display = "none"; $("emailConfirmErr").textContent = "";
 $("emailErr").style.display = "none"; $("emailErr").textContent = "";
 $("emailModal").setAttribute("aria-hidden","false");
 setTimeout(() => $("emailConfirm").focus(), 50);
}
...
async function sendQuoteEmail(){
 ...
 const re = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
 if (!re.test(email)){
  $("emailConfirmErr").textContent = "Enter a valid email address.";
 ...
 try {
  const inputs = currentInputs();
  const inputsWithExtras = {
   ...inputs,
   subjectZip: normalizeZip($("zip").value),
   value: Number($("value").value || 0),
   equity: Number($("equity").value || 0)
  };
  const cardData = computeCardData(lastQuote, inputs);
  const payload = {
   action: "sendQuote",
   leadToken,
   email,
   inputs: inputsWithExtras,
   quote: lastQuote,
   card: cardData,
   leadMeta: {
    first: localStorage.getItem("azm_leadFirst") || "",
  ...
  if (DEV_SHOW){ captureDev("devSaveReq", payload); setDevBlock("devSaveRes","// sending…"); }
  const res = await fetch(WS4_LOG, {
   method: "POST",
   headers: { "Content-Type": "text/plain" }, // avoid preflight (Apps Script doesn't handle OPTIONS)
   body: JSON.stringify(payload)
  });
  const data = await res.json();
  if (DEV_SHOW) captureDev("devSaveRes", data);
  if (!res.ok || !data.ok){
  ...
  closeEmailModal();
  toast("Quote sent to " + email,"success");
  showStatus("Quote emailed.","ok");
  return true;
 } catch (err) {
  const msg = err?.message || "Network error";
  $("emailErr").textContent = msg; $("emailErr").style.display = "block";
  if (DEV_SHOW) captureDev("devSaveRes", { ok: false, error: msg });
  return false;
 } finally {
...
function upsertLead(){
 ...
 const t = (typeof crypto !== "undefined" && crypto.randomUUID)
  ...
  : Array.from(crypto.getRandomValues(new Uint8Array(12))).map(x=>x.toString(16).padStart(2,"0")).join("").slice(0,12);
 leadToken = t;
  localStorage.setItem("azm_leadToken", leadToken);
  localStorage.setItem("azm_leadEmail", v.email);
  localStorage.setItem("azm_leadFirst", v.first);
  localStorage.setItem("azm_leadLast", v.last);
...
function bindEvents(){
 ...
 // Taxes & Insurance — live update only when not core-dirty (unchanged behavior)
 ["taxes","ins","hoa"].forEach(id => $(id).addEventListener("input", () => {
  if(!gated) return;
  if(!coreDirty) maybeReprice();
  else showRecalcOrGateStatus();
 }));
 ...
 // Points — always live update from cache (this is the key)
